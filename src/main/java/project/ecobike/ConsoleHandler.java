...
public class ConsoleHandler extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            ...
            CommandServices commandServices = menuService.getCommand(commandInput);
            if ((1 == commandInput) || (5 == commandInput)) {
                try {
                    commandServices.execute(catalogOfBikes, pathToFile);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            } else {
                try {
                    commandServices.execute(storageBikes, pathToFile);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
